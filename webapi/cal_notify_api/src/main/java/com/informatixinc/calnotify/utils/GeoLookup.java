	...
	public Point latLongFromAddress(Address address) {
		...
		try {
			...
			if (jsonResponse.get("status").getAsString().equals("OK")) {
				...
				point.setLatitude(firstMatch.get("geometry").getAsJsonObject().get("location").getAsJsonObject()
						.get("lat").getAsDouble());
				point.setLongitude(firstMatch.get("geometry").getAsJsonObject().get("location").getAsJsonObject()
						.get("lng").getAsDouble());
				return point;
			} else {
				return null;
